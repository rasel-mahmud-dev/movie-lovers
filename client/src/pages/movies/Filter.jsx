...
const Filter = (props) => {
    ...
    const topSpaceRef = React.useRef()
    ...
    function calcSpace(){
        if(accordionRef.current && topSpaceRef.current){
            let s = topSpaceRef.current.offsetHeight + accordionRef.current.offsetHeight;
            accordionRef.current.style.height = (s + 200) + "px"
        }
    }
                ...
                <div className="mt-2" key={key}>
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                        {filterItems[key] && filterItems[key].map(item=>(
                            <div key={item._id} className="bg-primary px-2 text-gray-200 rounded text-sm">
                                <span>{item.name}</span>
                            </div>
                        ))}
                    </div>
